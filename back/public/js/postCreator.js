window.onload = () => {
  ...
  pbtn.onclick = async function (event) {
    ...
    let createPostSection = document.getElementsByClassName(
      "create-post-section"
    );
  ...
  for (let i = 0; i < cbtn.length; i++) {
    cbtn[i].onclick = async function (event) {
      let text = document.getElementsByClassName("write-commentPrompt")[i]
        .innerText;
  ...
  for (let i = 0; i < editBtn.length; i++) {
    editBtn[i].onclick = async function (event) {
      ...
      let text =
        document.getElementsByClassName("edit-commentPrompt")[i].innerText;
      let ref_post_id = document.getElementsByClassName("hidden")[i].innerText;
      console.log(text);
